...
base_map <- ggplot(joined_precip_geom) +
  ...
  labs(title= "5-year precipitation comapred to the 20th century average", 
       subtitle = "January 2020-December 2024", 
       caption = "Source: National Centers for Environmental Education") +
  theme_void() +
...
my_brew_palette_11 <- RColorBrewer::brewer.pal(n=11, name= "BrBG")
```
...
base_map +
  scale_fill_gradientn(colors = my_brew_palette_11,
                       labels = label_percent(scale =1),
                       breaks = breaks_width(width = 10),
                       values = rescale(x = c(-40,0,20)))+
  guides(fill = guide_colorbar(barwidth = 25, barheight = .75))
```
...
# save HEX codes as vector ----
my_brew_palette_10 <- brewer.pal(n= 10, name = 'BrBG')
```
...
base_map +
  scale_fill_stepsn(colors = my_brew_palette_10,
                    labels = label_percent(scale=1),
