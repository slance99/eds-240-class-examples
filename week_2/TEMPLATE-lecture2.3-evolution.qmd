...
-   Problem of this Plot: spaghetti, too jumbled to understand
...
ggplot(lyme_pop, aes(x= year, y = cases_per100k, group = state)) + #each line represents a state 
  geom_line() +
  geom_point() #adds our points to this to be visible 
```
...
For some really good examples of why this is ok, look at the week 2 slides
...
lyme_pop |> 
  ...
  geom_line() +
  scale_y_continuous(breaks = seq(0, 190, by = 1)) +
  coord_fixed(ratio = 1) #each unit on x and y are the same, ggplot does not go with this fixed aspect ratio but fits the data to the box 
```
...
lyme_pop |> 
  ...
  ggplot(aes(x = year, y = cases_per100k, group = state)) +
  geom_line() +
  coord_fixed(ratio = 1/50) #spreads out plot more than default, for every 1 unit of heigh there are 50 units of length on the x 
```
...
lyme_pop |> 
  ...
  ggplot(aes(x=year, y = cases_per100k, group = state)) +
  geom_area() +
  scale_x_discrete(expand = c(0, NA)) + #fits all the data on the plot, starts the x axis axis now, discrete because year is discrete
  scale_y_continuous(expand = c(0, NA)) #starts y data at 0, continuous because lyme disease is continous 
```

## Stacked area charts

-   show evolution of the whole + relative proportions of each group
-   Note: when stack the thickness of each color is the amount of cases
...
# stacked area chart ----
lyme_pop |> 
  filter(state %in% c("Maine", "Rhode Island", "New Hampshire", "Tennessee")) |>
...
lyme_pop |> 
  ...
  ggplot(aes(x = year, y = cases_per100k, group = state, fill = state)) +
  geom_area(position = "fill") #makes it proportional, can see relative contributions of each states case count 
```
