...
jobs_clean |>
  filter(year == 2016) |>
  slice_max(order_by = total_earnings, n=10)  |>#take the top 10 observations when ordered by earning
  mutate(occupation = fct_reorder(occupation, total_earnings)) |>
  ggplot(aes(x= occupation, y = total_earnings)) +
  theme_classic() +
  scale_y_continuous(labels = label_currency(scale = 0.001, suffix = "k")) + #change things from 50000 to $50,000 to $50k
  geom_col() +
  geom_text(aes(label = dollar(total_earnings)), 
            hjust = 1.2,
            color = "white") + #add total earnings annotation
  coord_flip() 
...
jobs_clean |> 
  ...
  mutate(occupation = fct_reorder(occupation, total_earnings)) |> 
  ggplot(aes(x = occupation, y = total_earnings)) +
  geom_point() + #places top points 
  geom_linerange(aes (ymin = 0,
                      ymax = total_earnings)) + #places horizontal line, defines beginning and end point 
  coord_flip() +
  scale_y_continuous(labels = label_currency(scale = 0.001, suffix = "k")) + #change things
  theme_classic() +
    geom_text(aes(label = dollar(total_earnings)), 
            hjust = 1,
            vjust = -.8,
            color = "black") + #add total earnings annotation
    theme_bw()
...
jobs_clean |> 
  ...
  pivot_longer(cols = c(total_earnings_female, total_earnings_male), 
               names_to = "group", 
               values_to = "earnings_by_group") |> #pivot longer to make two new columns 
  mutate(sex = str_remove(string = group, pattern = "total_earnings_")) |> #clip off total earnings words 
...
jobs_clean |>
  ...
  geom_point(position = position_dodge(width = 0.5)) + #have to define a width for the dodge command 
  geom_linerange(aes(ymin = 0, ymax = earnings_by_group), position = position_dodge(width = 0.5)) +
  coord_flip()
```
...
ggplot(heatmap_order, aes(x=year, y = occupation, fill = total_earnings)) +
  geom_tile() +
  coord_fixed() + #forces x and y units to be equal to make perfect squares 
  scale_fill_viridis_c(labels = label_currency(scale = 0.001, suffix = "k")) +
  scale_y_discrete(labels = label_wrap(30)) +#number of characters before wrapping to another line 
  labs(fill = "Total Earnings") +
  theme_minimal() +
  guides(fill = guide_colorbar(barwidth = 15,
                               barheight = 0.75,
                               title.position = "top")) + #changes how this looks so its better on top 
